...
getwd()
library(dplyr)
shrub_experiments <- read.csv("../raw-data/shrub-volume-experiments.csv") 
shrub_volume <- read.csv("../raw-data/shrub-volume-data.csv")
shrub_sites <- read.csv("../raw-data/shrub-volume-sites.csv")
colnames(shrub_sites)
```
...
```{r}
shrub_combined <- inner_join(shrub_experiments, shrub_volume, by = "experiment")
colnames(shrub_combined)
```
#4.2
...
```{r}
shrub_full <- inner_join(shrub_combined, shrub_sites, by = "site")
colnames(shrub_full)
```
...
```{r}
#finding the smallest values of length, width, and height using the min() function
#and removing all NA values to get an accurate answer with the, na.rm = TRUE command 
min(length, na.rm = TRUE) #1.1 
min(width, na.rm = TRUE) # 0.5
min(height, na.rm = TRUE) #1.5 

```

```{r}
#finding the largest values of length, width, and height using the max() function
#and removing all NA values to get an accurate answer with the, na.rm = TRUE command 
max(length, na.rm = TRUE) #3.5
max(width, na.rm = TRUE) #4.5
max(height, na.rm = TRUE) #9.6

```

```{r}
#finding the sum of the values in length, width, and height separetly using the sum() function
#and removing all NA values to get an accurate answer with the, na.rm = TRUE command 
sum(length, na.rm = TRUE) #25
sum(width, na.rm = TRUE) #19.6
sum(height, na.rm = TRUE) #45.4 
```

```{r}
#finding the mean of the values in length, width, and height using the mean function
#and removing all NA values to get an accurate answer with the, na.rm = TRUE command 
mean(length, na.rm = TRUE) #2.5
mean(width, na.rm = TRUE) #2.18
mean(height, na.rm = TRUE) #4.54 
```
...
```{r}
vectors_volumes<-length * width * height 
# 27.456  35.112   8.910  20.250  38.440   NA 15.390  1.265  52.500  25.056
```

```{r}
#finding the sum of the volume of all of the shrubs. with the sum function with the input name as the vector saved in the step above and using the na.rm= TRUE input to obtain an answer with the NA values removed 
sum(vectors_volumes, na.rm = TRUE) # 224.379
```

```{r}
#A vector of the height of shrubs with lengths > 2.5. using the subset(vector or data frame name, name of column or vector we want to dissect to find values either  than a number in this case 2.5 ) function i was able to get all the heights higher that 2.5
height_above_2.5 <- subset( height, height > 2.5) #9.6 7.6 4.0 3.0 4.5 7.5 3.2
height_above_2.5
```

```{r}
#A vector of the height of shrubs with heights > 5.used same steps as explained above 
height_above_5 <- subset( height, height > 5)#9.6 7.6 7.5
height_above_5
```

```{r}
#A vector of the heights of the first 5 shrubs (using []).
first_five_hights <- height[1:5] #9.6 7.6 2.2 1.5 4.0

```

```{r}
#A vector of the volumes of the first 3 shrubs (using []). this function allows me to select what number to : what number i want 

first_three_volumes <- vectors_volumes[1:3]#27.456 35.112  8.910
...
```{r}
#used the tail() it is used to display the last n rows of the dataset or vector being used 
tail(vectors_volumes,5)# NA 15.390  1.265 52.500 25.056
```
# 6
```{r}
dimension <- read.csv("../raw-data/shrub-dimensions-labeled.csv")
colnames(dimension)
```

```{r}
#A vector of shrub lengths the $ gives you the values in a specific dataset "dataset name$column name"
dimensions_length <- dimension$length #2.2 2.1 2.7 3.0 3.1 2.5 1.9 1.1 3.5 2.9
dimensions_length
```

```{r}
#A vector of the volume of each of the shrubs ised the $ to select eac colum needed to calculate the volume 
dimension_volumes <- dimension$length * dimension$width * dimension$height #27.456 35.112  8.910 20.250 38.440 21.000 15.390 1.265 52.500 25.056
```

```{r}
#to Create data frame with just the shrubID and height columns i used the setect() function and inside put the name of the data set where we are getting the columns and the column names i wanted in it 
shrubID_height <- select(dimension, shrubID, height)

```

```{r}
#A data frame with the second row of the full data frame using the 'name of data frame where the columns and rows come from[rows ex 1:3 , columns blank for all columns ]
second_row <- dimension[2,]
second_row
```

```{r}
#A data frame with the first 5 rows of the full data frame used the dame explanation above 
first_five_rows<- dimension[1:5,]
first_five_rows
```
